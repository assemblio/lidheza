...
def index(pid):
    ...
    if len(publisher['adSpaces']) == 0:
        ...
        # Get the campaigns
        finished_campaigns = mongo_utils.get_publisher_finished_campaigns(pid)
        published_campaigns = mongo_utils.get_publisher_published_campaigns(pid)
        paused_campaigns = mongo_utils.get_publisher_paused_campaigns(pid)
...
def update_impression_rate(pid):
    rate_form = ImpressionRateForm(request.form)
    if not rate_form.validate():
        flash_errors(rate_form)
        return redirect(url_for('publisher.index', pid=pid))
    else:
        mongo_utils.update_impression_rate(pid, float(rate_form.rate.data))
        return redirect(url_for('publisher.index', pid=pid))
...
def campaign_create(pid):
    ...
    if request.method == 'GET':
        ...
        if not form.validate():
            ...
            campaign = {
                'advertiserEmail': form.advertiser_email.data,
                'publisher': {
                    'id': pid,
                    'name': publisher['name'],
                    'host': publisher['host']
...
def campaign_edit(pid, campaign_id):
    ...
    if request.method == 'GET':
        ...
        form.advertiser_email.data = campaign['advertiserEmail']
        ...
        # POST: We are submitting an edit to the campaign.

        form = CampaignForm(request.form)

        if campaign['status'] == 'published' or campaign['status'] == 'paused':
            # If the campaign is already published or has been published and is currently paused,
            # we can only edit: advertiser email, name, url, end date, and impressions goals.
            update = {
                'advertiserEmail': form.advertiser_email.data,
                'name': form.campaign_name.data,
                'url': form.url.data,
...
@mod_publisher.route('/<pid>/campaign/<campaign_id>/pause', methods=['POST'])
def campaign_pause(pid, campaign_id):
    mongo_utils.set_campaign_as_paused(campaign_id)
    return redirect(url_for('publisher.index', pid=pid))
...
def settings_adspaces(pid):
    if request.method == 'GET':
        ...
        available_ad_spaces = []
        if form.available_ad_spaces.data is not None:
            available_ad_spaces = form.available_ad_spaces.data

        mongo_utils.update_publisher_available_ad_spaces(pid, available_ad_spaces)
...
def upload_campaign_asset(pid, campaign_id):
    ...
    try:
        if file and allowed_file(file.filename):
            ...
            # TODO: Check if image dimensions are correct.
            # Continue if they are, cancel image file creation if they are not.
            ...
            try:
                ...
                return ('An unexpected error has occurred. Please contact site administrator.', 500)
